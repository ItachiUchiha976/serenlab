/* ============================================================
   BOS reveal au scroll — SérénLab — 16/07/2026
   ...
   Adapte de la couche Curiosa aux classes reelles SérénLab
   (product-card__* identiques a Curiosa ; promise-card/quiz-teaser-card/
   capture-card en plus ; pas de bundle-card/reassurance/story/vip-strip).
   ============================================================ */
(function () {
  try {
    ...
    var selectors = [
      '.product-card', '.promise-card', '.trust-item',
      '.section-title', '.section-sub',
      '.quiz-teaser-card', '.capture-card'
    ];
    ...
    ['.products-grid', '.promise-grid'].forEach(function (gSel) {
    ...
    /* ---- Barre d'achat sticky mobile (fiches produit) ----
       Construite depuis le 1er bouton d'ajout panier de la page : zero
       duplication de logique (le clic proxifie le vrai bouton, donc
       panier + Stripe + compteurs restent la source unique). SérénLab
       n'a pas d'attribut [data-add-cart] (contrairement a Curiosa) :
       ses boutons portent la classe .add-to-cart-btn + data-name/data-price
       -> on essaie les deux selecteurs. Si l'ancienne barre
       bos-sticky-cta.js tourne encore sur une page (residu), on ne
       construit pas la notre par-dessus pour eviter 2 barres empilees. */
    (function stickyAtc() {
